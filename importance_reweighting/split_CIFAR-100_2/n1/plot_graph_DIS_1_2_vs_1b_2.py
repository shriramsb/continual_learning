...
for i in range(num_perms):
	with open('DIS1b_2/' + summaries_path_prefix + str(i) + '/test_accuracy', 'rb') as f:
...
plt.errorbar(x, test_accuracies_mean, yerr=test_accuracies_std, label='DIS1b_2')
...
# print("Mean test accuracy across tasks :", np.mean(test_accuracies_mean))
x = list(range(class_per_task, num_class + 1, class_per_task))
plt.errorbar(x, test_accuracies_mean, yerr=test_accuracies_std, label='DIS1_2')

plt.legend()
plt.savefig('test_DIS_1_2_vs_DIS_1b_2' + str(class_per_task) + '.png')