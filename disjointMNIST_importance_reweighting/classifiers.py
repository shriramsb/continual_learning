...
# every function gets feed_dict as an argument (except ones managing EWC). So, need to pass weights of examples from tuner
...
class Classifier(object):
    def __init__(self, network, input_shape, output_shape, checkpoint_path):
        ...
        self.x_fisher = None
        self.y_fisher = None
        self.fisher_average_scale = None
    ...
    def createLossAccuracy(self):
        with tf.name_scope("loss"):
            average_nll = tf.reduce_sum(self.loss_weights * tf.nn.softmax_cross_entropy_with_logits(logits=self.scores, labels=self.y)) / tf.reduce_sum(self.loss_weights)  # wat abt just softmax_cross_entropy
    ...
    def createTrainStep(self, fisher_multiplier):
        with tf.name_scope("optimizer"):
            ...
            if self.use_ewc:
                ...
                self.loss_with_penalty = self.loss
            if self.use_ewc:
